...
const DATABASE_COLUMNS = [
  ...
  { value: "areas_covered", label: "Areas Covered" },
  { value: "service_frequency", label: "Service Frequency" },
  { value: "service_active_inactive", label: "Service Status" },
  { value: "invoice_type", label: "Invoice Type" },
  { value: "cpm_device_onsite", label: "CPM Device Onsite" },
  { value: "unit_price", label: "Unit Price" },
  { value: "cpm_pricing", label: "CPM Pricing" },
  { value: "cpi", label: "CPI" },
...
interface ServiceAgreementImportDialogProps {
  onSuccess?: () => void;
}

export function ServiceAgreementImportDialog({ onSuccess }: ServiceAgreementImportDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      const allMappedData = csvData.map((row, index) => {
        ...
        Object.entries(columnMapping).forEach(([csvCol, dbCol]) => {
          if (dbCol !== "skip" && row[csvCol]) {
            ...
            if (["unit_price", "cpm_pricing", "cpi", "total"].includes(dbCol)) {
              const numValue = parseFloat(row[csvCol]);
              if (!isNaN(numValue)) {
                obj[dbCol] = numValue;
              }
            } else {
      ...
      if (dataToInsert.length === 0) {
        setError("No valid rows to import. All rows are missing required fields (Service ID).");
        return;
      }

      const { error } = await supabase.from("service_agreements").insert(dataToInsert);

      if (error) throw error;

      let successMessage = `${dataToInsert.length} service agreements imported successfully`;
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Import Service Agreement Data - {step === "upload" ? "Step 1: Upload File" : "Step 2: Map Fields"}</DialogTitle>
          <DialogDescription>
            {step === "upload" 
              ? "Upload a CSV or XLSX file with service agreement data. The first row should contain column headers."
